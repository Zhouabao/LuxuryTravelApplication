...
public class VoisePlayingIcon extends View {
    ...
    public VoisePlayingIcon(Context context, @Nullable AttributeSet attrs) {
        ...
        pointerNum = ta.getInt(R.styleable.voisePlayingIconAttr_pointer_num, 11);//指针的数量，默认为4
        pointerWidth = SizeUtils.dp2px(ta.getFloat(R.styleable.voisePlayingIconAttr_pointer_width, 5f));//指针的宽度，默认5dp
    ...
    public VoisePlayingIcon(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.voisePlayingIconAttr);
        pointerColor = ta.getColor(R.styleable.voisePlayingIconAttr_pointer_color, Color.RED);
        pointerNum = ta.getInt(R.styleable.voisePlayingIconAttr_pointer_num, 11);
        pointerWidth = SizeUtils.dp2px(ta.getFloat(R.styleable.voisePlayingIconAttr_pointer_width, 5f));
        pointerSpeed = ta.getInt(R.styleable.voisePlayingIconAttr_pointer_speed, 40);
        init();
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        for (int i = 0; i < pointerNum; i++) {
            ...
            pointers.add(new Pointer((float) (0.8 * (random.nextInt(10) + 1) * (getHeight() - getPaddingBottom() - getPaddingTop()))));
        }
        //计算每个指针之间的间隔  总宽度 - 左右两边的padding - 所有指针占去的宽度  然后再除以间隔的数量
        pointerPadding = SizeUtils.dp2px(13F);
...
//                    basePointY,
//                    paint);
            canvas.drawRoundRect(basePointX,
                    basePointY - pointers.get(i).getHeight(),
                    basePointX + pointerWidth,
                    basePointY,
                    SizeUtils.dp2px(10F), SizeUtils.dp2px(10F),
                    paint);
